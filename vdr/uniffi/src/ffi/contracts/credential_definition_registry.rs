...
use indy_besu_vdr::{credential_definition_registry, Address, CredentialDefinitionId};
use serde_json::json;
...
pub async fn resolve_credential_definition(client: &LedgerClient, id: &str) -> VdrResult<String> {
    let schema = credential_definition_registry::resolve_credential_definition(
        &client.client,
        &CredentialDefinitionId::from(id),
    )
    .await?;
