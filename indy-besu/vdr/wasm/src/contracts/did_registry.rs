use indy2_vdr::{DID, DidDocument, IndyDidRegistry, Address};
...
pub struct IndyDidRegistryWrapper(pub(crate) IndyDidRegistry);

#[wasm_bindgen(js_class = IndyDidRegistry)]
impl IndyDidRegistryWrapper {
    ...
    pub async fn build_create_did_transaction(client: &LedgerClientWrapper,
        ...
        let transaction = IndyDidRegistry::build_create_did_transaction(&client.0, &address, &did_doc).await.as_js()?;
    ...
    pub async fn build_update_did_transaction(client: &LedgerClientWrapper,
        ...
        let did_doc: DidDocument = serde_wasm_bindgen::from_value(did_doc)?;
        let address = Address::new(from);
        let transaction = IndyDidRegistry::build_update_did_transaction(&client.0, &address, &did_doc).await.as_js()?;
        Ok(TransactionWrapper(transaction))
    }
    ...
    pub async fn build_deactivate_did_transaction(client: &LedgerClientWrapper,
        ...
        let address = Address::new(from);
        let did = DID::new(did);
        let transaction = IndyDidRegistry::build_deactivate_did_transaction(&client.0, &address, &did).await.as_js()?;
        Ok(TransactionWrapper(transaction))
    }
    ...
    pub async fn build_resolve_did_transaction(client: &LedgerClientWrapper,
                                               did: &str) -> Result<TransactionWrapper> {
        let did = DID::new(did);
        let transaction = IndyDidRegistry::build_resolve_did_transaction(&client.0, &did).await.as_js()?;
        Ok(TransactionWrapper(transaction))
    }
    ...
    pub fn parse_resolve_did_result(client: &LedgerClientWrapper,
        ...
        let did_doc = IndyDidRegistry::parse_resolve_did_result(&client.0, &bytes).as_js()?;
