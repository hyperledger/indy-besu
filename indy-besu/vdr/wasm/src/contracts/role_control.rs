...
impl RoleControl {
    ...
    pub async fn build_assign_role_transaction(client: &LedgerClientWrapper,
        ...
        let transaction = role_control::build_assign_role_transaction(&client.0, &from, &role, &account).await.as_js()?;
    ...
    pub async fn build_revoke_role_transaction(client: &LedgerClientWrapper,
        ...
        let from = Address::from(from);
        let account = Address::from(account);
        let transaction = role_control::build_revoke_role_transaction(&client.0, &from, &role, &account).await.as_js()?;
        Ok(TransactionWrapper(transaction))
    }
    ...
    pub async fn build_has_role_transaction(client: &LedgerClientWrapper,
        ...
        let role = Role::try_from(role).as_js()?;
        let account = Address::from(account);
        let transaction = role_control::build_has_role_transaction(&client.0, &role, &account).await.as_js()?;
        Ok(TransactionWrapper(transaction))
    }
    ...
    pub async fn build_get_role_transaction(client: &LedgerClientWrapper,
                                            account: &str) -> Result<TransactionWrapper> {
        let account = Address::from(account);
        let transaction = role_control::build_get_role_transaction(&client.0, &account).await.as_js()?;
        Ok(TransactionWrapper(transaction))
    }
