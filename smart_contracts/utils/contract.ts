...
export class Contract {
  ...
  public async deployProxy(options?: { params?: any; libraries?: [Contract] }) {
    ...
    const factory = await ethers.getContractFactory(this.name, {
      signer: this.signer,
      libraries: this.toDictionary(libraries),
    })

    this.instance = await upgrades.deployProxy(factory, params ?? [], {
      kind: 'uups',
  ...
  public async deploy(options?: { params?: any; libraries?: [Contract] }) {
    ...
    this.instance = await ethers.deployContract(this.name, params ?? [], {
      signer: this.signer,
